<template >
  <div class="q-pa-sm">
    <form @submit.prevent="submit">
      <!---------------------------------------- select data source ---------------------------------------->
      <div class="q-my-sm">
        <div class="text-uppercase q-ma-xs">WIND EROSION SENSITIVITY MAPS</div>
...
export default {
  data() {
    return {
      form_data: {
        ...
        // default indicator
        indicator: {
          label: "Wind Erosion Sensitivity Index",
          value: "ilswe",
          computation_type: "Ilswe",
          colors: colors
        },
  ...
  computed: {
    qualityIndices() {
      return [
        {
          label: "Climate Erosivity",
          value: "climate_erosivity",
          colors: colors,
          computation_type: "Climate Erosivity"
        },
        {
          label: "Soil Erodible Fraction",
          value: "soil_erodible_fraction",
          colors: colors,
          computation_type: "Erodible Fraction"
        },
        {
          label: "Soil Crust Factor",
          value: "soil_crust_factor",
          colors: colors,
          computation_type: "Soil Crust"
        },
        {
          label: "Roughness Factor",
          value: "roughness_factor",
          colors: colors,
          computation_type: "Soil Roughness"
        },
        {
          label: "Vegetation Cover",
          value: "vegetation_cover",
          colors: colors,
          computation_type: "Vegetation Cover"
        },
        {
          label: "Wind Erosion Sensitivity Index",
          value: "ilswe",
  ...
  methods: {
    ...
    async fetchYears() {
      this.form_data.years = await fetchComputationYears("ILSWE")
    },
    async submit() {
      try {
        let payload = {
        ...
        const uri = `/api/ilswe/`; // build the request url
